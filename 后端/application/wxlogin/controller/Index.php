...
class Index
{
    ...
    //默认配置
    protected $config = [
        'url' => "session_key url", //微信获取session_key接口url
    ...
    public function __construct()
    {
        ...
        if ($wx = Config::get('wx_session_key')) {
    ...
    {
        /**
         * 1.小程序调用server获取token接口, 传入code, rawData, signature, encryptedData.
         */
        $code = input("post.code", '', 'htmlspecialchars_decode');
        $rawData = input("post.rawData", '', 'htmlspecialchars_decode');
        $signature = input("post.signature", '', 'htmlspecialchars_decode');
        $encryptedData = input("post.encryptedData", '', 'htmlspecialchars_decode');
        $iv = input("post.iv", '', 'htmlspecialchars_decode');
        ...
        if ($res['code'] !== 200 || !isset($res['result']) || !isset($res['result'])) {
            return GlobalVariable::promptErrorByJSON("错误", ErrorCode::$RequestTokenFailed, "请求Token失败！");
        }
        ...
        /**
         * 3.server计算signature, 并与小程序传入的signature比较, 校验signature的合法性, 不匹配则返回signature不匹配的错误. 不匹配的场景可判断为恶意请 求, 可以不返回.
        ...
        if ($signature2 !== $signature)
            return GlobalVariable::promptErrorByJSON("错误", ErrorCode::$SignNotMatch, "签名不匹配！");

        /**
         *
         * 4.使用第1步返回的session_key解密encryptData, 将解得的信息与rawData中信息进行比较, 需要完全匹配,
        ...
        if ($errCode !== 0) {
            return GlobalVariable::promptErrorByJSON("错误", ErrorCode::$EncryptDataNotMatch, "解密信息错误！");
        }
...
//        $data['realName'] = $realName;
        ...
        $user = new User();
        $status = $user->judgeInfo($data);
        return GlobalVariable::promptData($status);
    }
    ...
    public function login()
    {
        ...
        $encryptedData = input("post.encryptedData", '', 'htmlspecialchars_decode');
        $iv = input("post.iv", '', 'htmlspecialchars_decode');
        $phoneNumber = input("post.phoneNumber", '', 'htmlspecialchars_decode');
        $realName = input("post.realName", '', 'htmlspecialchars_decode');
        ...
        /**
         * 记录用户登录信息，首次使用的用户记录用户信息。
         */
        $user = new User();
        $status = $user->setInfo($data);
        $data['status'] = $status;
        $user->login($data);
    ...
    {
        /**
         * step 1: 小程序获取用户联系电话权限, 传入encryptedData, iv, session3rd.
         */
        $encryptedData = input("post.encryptedData", '', 'htmlspecialchars_decode');
        $iv = input("post.iv", '', 'htmlspecialchars_decode');
        $session3rd = input("post.session", '', 'htmlspecialchars_decode');

        /**
         * step 2: 获取session_key
         */
        $sessionKeyFind = Db::name('user_login_record')->where([
            'session' => $session3rd
        ])->find();
        $sessionKey = $sessionKeyFind['session_key'];

        /**
         * step 3: 使用第2步返回的session_key解密encryptData, 将解得的信息与rawData中信息进行比较, 需要完全匹配,
         * 解得的信息中也包括openid, 也需要与第4步返回的openid匹配. 解密失败或不匹配应该返回客户相应错误.
         * （使用官方提供的方法即可）
         */
        $pc = new WXBizDataCrypt($this->config['appid'], $sessionKey);
        $errCode = $pc->decryptData($encryptedData, $iv, $data);
        if ($errCode !== 0) {
            return GlobalVariable::promptErrorByJSON("错误", 500, "解密信息错误！");
        }
        ...
        $user = Db::name('user_login_record')->where([
            'session' => $session3rd
        ])->find();
...
//        } else {
//            return GlobalVariable::promptErrorByJSON("错误", 500, "录入失败！");
//        }
    }
